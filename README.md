...
Within `app/main.py` the `/ping` endpoint supports optional fault injection for demo scenarios, but it is disabled by default so normal deployments return `200`/`{"pong": true}` consistently. Set `ENABLE_PING_FAULT_INJECTION=true` to re-enable the deliberate intermittent `500` response path for demos. If log aggregation, and even better, vector indexing of PRs is enabled, you can tie a prometheus or datadog alert directly to a full root cause using Plural AI, and it will even spawn a PR to fix the broken code change.
