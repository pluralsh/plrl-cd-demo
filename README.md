...
Within `app/main.py` this demo previously included a deliberately broken `/ping` endpoint to demonstrate AI-assisted root-cause analysis and remediation. That behavior has been removed so `/ping` now behaves as a stable health-style endpoint while the repository still serves as a simple Plural CD demo app.
