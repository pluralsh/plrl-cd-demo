...
`/ping` returns a successful `{ "pong": true }` response by default. To intentionally exercise error-alerting workflows, set `PING_FAULT_INJECTION=true`; only then does the original time-based fault injection raise `Exception("unknown internal error")` roughly once every three seconds.
