...
Within `app/main.py` the demo keeps an intentionally broken endpoint at `/simulate-error` for alerting/root-cause exercises, while `/ping` is reserved as a consistently healthy endpoint for probes and sidecars. If log aggregation, and even better, vector indexing of PRs is enabled, you can tie a prometheus or datadog alert directly to a full root cause using Plural AI, and it will even spawn a PR to fix the broken code change.
