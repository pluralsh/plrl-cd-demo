...
The `/ping` endpoint in `app/main.py` previously contained intentional error logic for demo purposes. With log aggregation and vector indexing of PRs enabled, Plural AI can tie prometheus or datadog alerts directly to full root cause analysis and even spawn PRs to fix broken code changes.
