...
Within `app/main.py` the `/ping` endpoint is now a stable health-style response that returns `{"pong": true}`. The intentional intermittent demo failure remains available under `/demo/fail-sometimes`, which preserves the synthetic 500 behavior without making `/ping` misleading for alerting or probes.
