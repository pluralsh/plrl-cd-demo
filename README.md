...
The `/ping` endpoint now returns success by default so the demo app remains healthy in continuously monitored environments. For demos that need to simulate the old intermittent 500s, set `PING_FAILURE_MODE=time-based` to re-enable the time-based fault injection behavior intentionally.
