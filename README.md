...
Within `app/main.py` the `/ping` endpoint is a stable health-style check that always returns `200` with `{"pong": true}`. For demoing alerting or AI-driven fixes, use the intentionally flaky `/demo/fail` endpoint instead.
