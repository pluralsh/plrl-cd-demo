...
The `/ping` endpoint provides a stable health check returning `{"pong": true}`. If log aggregation and vector indexing of PRs is enabled, you can tie a prometheus or datadog alert directly to a full root cause using Plural AI, and it can even spawn a PR to fix broken code changes.
