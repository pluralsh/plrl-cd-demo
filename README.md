...
Within `app/main.py` we've kept the health endpoint `/ping` stable and moved the deliberately broken demo behavior to `/ping/fail`. If log aggregation, and even better, vector indexing of PRs is enabled, you can still tie a prometheus or datadog alert directly to a full root cause using Plural AI and have it spawn a PR against the intentional failure path instead of the health check.
