...
The `GET /ping` endpoint is intended to be a stable smoke or health-style endpoint that returns `200` with `{"pong": true}`. If log aggregation, and even better, vector indexing of PRs is enabled, you can tie a prometheus or datadog alert directly to a root-cause workflow in Plural AI when a real regression occurs.
